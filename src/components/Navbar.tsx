...
export default function Navbar() {
    ...
    const contextTheme = useMemo(() => {
        switch (navContext) {
            case "admin": return { label: "ADMIN CONSOLE", color: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-600/10 dark:bg-emerald-400/10", border: "border-emerald-600/20 dark:border-emerald-400/20" };
            case "seller": return { label: "SELLER TERMINAL", color: "text-amber-600 dark:text-amber-400", bg: "bg-amber-600/10 dark:bg-amber-400/10", border: "border-amber-600/20 dark:border-amber-400/20" };
            case "shop": return { label: "AI ASSET SHOP", color: "text-secondary", bg: "bg-secondary/10", border: "border-secondary/20" };
            default: return null;
        }
    }, [navContext]);
    ...
    return (
        <nav
            ...
            className={cn(
                "fixed top-0 left-0 right-0 z-50 transition-all duration-700 px-4 md:px-0 flex flex-col items-center",
                                ...
                                <AnimatePresence>
                                    {link.submenu && activeDropdown === link.name && (
                                        <motion.div
                                            ...
                                            className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-72 rounded-2xl overflow-hidden py-4 z-[100] bg-card dark:bg-[#0f0f11] border border-border/50 dark:border-[#27272a] shadow-xl"
                                        >
                                            {/* Invisible Bridge */}
                                            <div className="absolute -top-4 left-0 right-0 h-4 bg-transparent" />
                                            
                                            <div className="relative px-6 py-2 mb-2 border-b border-border/50">
                                                <p className="text-[9px] font-black text-foreground/50 dark:text-foreground/20 uppercase tracking-[0.3em]">{link.label}</p>
                                            </div>
                                            {link.submenu.map((item: any) => (
                                                <Link
                                                    key={item.name}
                                                    href={item.href}
                                                    className="block px-8 py-3 text-foreground/70 dark:text-foreground/40 hover:text-foreground hover:bg-foreground/5 transition-all text-[10px] font-black uppercase tracking-[0.2em] relative group/item"
                                                >
                                                    <div className="flex items-center gap-4">
                                                        {item.icon && <span className="opacity-50 dark:opacity-30 group-hover/item:opacity-100 transition-all">{item.icon}</span>}
                                                        <span>{item.name}</span>
                                                    </div>
                                                    <div className="absolute left-4 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-secondary scale-0 group-hover/item:scale-100 transition-transform duration-300" />
                                                </Link>
                                            ))}
                                        </motion.div>
                                    )}
                ...
                <div className="flex items-center gap-4">
                        ...
                        <>
                            ...
                            {session ? (
                                <div className="relative group/profile"
                                    ...
                                    onMouseLeave={handleMouseLeave}>
                                    <button className="flex items-center gap-3 px-4 py-2 bg-foreground/5 border border-border rounded-full hover:bg-foreground/10 hover:border-foreground/20 transition-all active:scale-95 shadow-lg">
                                        <div className="w-6 h-6 rounded-full bg-gradient-to-tr from-zinc-800 to-zinc-600 flex items-center justify-center text-[10px] font-black text-foreground/90 uppercase border border-foreground/20">
                                            {session.user.name?.charAt(0) || "U"}
                                        </div>
                                        <span className="hidden sm:inline text-[10px] font-black text-foreground/50 group-hover/profile:text-foreground uppercase tracking-[0.2em] transition-colors">
                                    ...
                                    <AnimatePresence>
                                        {activeDropdown === "profile" && (
                                            <motion.div
                                                ...
                                                className="absolute top-full right-0 mt-3 w-72 backdrop-blur-3xl border border-border/50 dark:border-[#27272a] rounded-2xl shadow-[0_32px_64px_rgba(0,0,0,0.1)] dark:shadow-[0_32px_64px_rgba(0,0,0,1)] overflow-hidden py-3 z-[100] bg-card dark:bg-[#0f0f11]"
                                            >
                                                {/* Invisible Bridge */}
                                                <div className="absolute -top-4 left-0 right-0 h-4 bg-transparent" />
                                                
                                                <div className="px-6 py-4 border-b border-border/50 mb-3 hover:bg-foreground/[0.02] transition-colors cursor-default">
                                                    <p className="text-[10px] font-black text-secondary dark:text-foreground/30 uppercase tracking-[0.3em] mb-1.5 flex items-center gap-2">
                                                        <User size={10} /> Authenticated Access
                                                    </p>
                                                    <p className="text-[12px] font-bold text-foreground/90 truncate">{session.user.name}</p>
                                                    <p className="text-[9px] font-medium text-foreground/50 dark:text-foreground/30 truncate mt-1">{session.user.email}</p>
                                                </div>
                                                ...
                                                {isAdmin && (
                                                    <div className="pb-2">
                                                        ...
                                                        <Link href="/admin" className="flex items-center gap-3 px-8 py-3 text-foreground/70 hover:text-foreground hover:bg-emerald-400/5 transition-all text-[10px] font-black uppercase tracking-[0.2em]">
                                                            Master Dashboard
                                                        </Link>
                                                    </div>
                                                )}
                                                ...
                                                {isRegisteredSeller ? (
                                                    <div className="pb-2 border-t border-border pt-2">
                                                        ...
                                                        <Link href="/seller" className="flex items-center gap-3 px-8 py-3 text-foreground/70 hover:text-foreground hover:bg-amber-400/5 transition-all text-[10px] font-black uppercase tracking-[0.2em]">
                                                            Seller Dashboard
                                                        </Link>
                                                        <Link href="/seller/products" className="flex items-center gap-3 px-8 py-3 text-foreground/70 hover:text-foreground hover:bg-amber-400/5 transition-all text-[10px] font-black uppercase tracking-[0.2em]">
                                                            Inventory Control
                                                        </Link>
                                                    </div>
                                                ) : (
                                                    <div className="pb-2 border-t border-border pt-2">
                                                        <p className="px-6 py-2 text-[9px] font-black text-foreground/20 uppercase tracking-[0.3em] flex items-center gap-3">
                                                            <Rocket size={12} /> Network Expansion
                                                        </p>
                                                        <Link href="/become-seller" className="mx-4 mt-1 flex items-center justify-center gap-3 px-6 py-3.5 bg-gradient-to-r from-zinc-800 to-zinc-900 border border-border rounded-xl text-foreground hover:from-white hover:to-white hover:text-background transition-all duration-500 text-[10px] font-black uppercase tracking-[0.2em] shadow-xl">
                                                            <Zap size={14} className="text-amber-400 group-hover:text-background" /> Join as Seller
                                                        </Link>
                                                    </div>
                                                )}

                                                <div className="py-2 border-t border-border">
                                                    <p className="px-6 py-2 text-[9px] font-black text-foreground/20 uppercase tracking-[0.3em]">Personal Access</p>
                                                    <Link href="/admin/settings" className="flex items-center gap-4 px-8 py-3 text-foreground/60 hover:text-foreground hover:bg-foreground/5 transition-all text-[10px] font-black uppercase tracking-[0.2em]">
                                                        <Settings size={12} /> Account Config
                                                    </Link>
                                                    <Link href={isAdmin ? "/admin/orders" : "/shop"} className="flex items-center gap-4 px-8 py-3 text-foreground/60 hover:text-foreground hover:bg-foreground/5 transition-all text-[10px] font-black uppercase tracking-[0.2em]">
                                                ...
                                                <button
                                                    ...
                                                    className="w-full flex items-center gap-4 px-8 py-4 mt-2 border-t border-border text-red-400 hover:text-red-500 hover:bg-red-400/5 transition-all text-[10px] font-black uppercase tracking-[0.25em] group/logout"
                                                >
                                ...
                                <div className="flex items-center gap-3">
                                    <Link href="/sign-in" className="hidden sm:inline-block">
                                        <button className="text-foreground/70 dark:text-foreground/40 hover:text-foreground px-5 py-2.5 rounded-full text-[10px] font-black uppercase tracking-[0.2em] transition-all hover:bg-foreground/5">
                                            Login
                                        </button>
                                    </Link>
                                    <Link href="/sign-up">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            ...
                            className="lg:hidden p-3 bg-secondary/5 border border-secondary/10 rounded-2xl text-foreground/50 hover:text-foreground transition-all active:scale-90 shadow-inner"
                        >
            ...
            <AnimatePresence>
                {mobileMenuOpen && (
                    <motion.div
                        ...
                        className="lg:hidden w-full max-w-sm mt-4 bg-card dark:bg-[#0f0f11] backdrop-blur-3xl border border-border rounded-[2.5rem] overflow-hidden shadow-[0_32px_64px_rgba(0,0,0,0.15)] dark:shadow-[0_32px_64px_rgba(0,0,0,0.9)] relative"
                    >
                            ...
                            {navLinks.map((link) => (
                                <div key={link.name} className="space-y-6">
                                    <div className="flex items-center justify-between group">
                                        <Link
                                            href={link.href}
                                            className="flex items-center gap-4 text-foreground/60 hover:text-foreground font-black py-1 uppercase tracking-[0.35em] text-[11px] transition-all"
                                            onClick={() => setMobileMenuOpen(false)}
                                        >
                                            {link.icon || <div className="w-2 h-2 rounded-full bg-foreground/10 group-hover:bg-foreground transition-colors" />}
                                            {link.name}
                                        </Link>
                                        {link.submenu && (
                                            <button
                                    ...
                                    <AnimatePresence>
                                        {link.submenu && activeMobileDropdown === link.name && (
                                            <motion.div
                                                ...
                                                className="overflow-hidden ml-6 space-y-5 border-l-2 border-border pl-8 mt-4"
                                            >
                                                {link.submenu.map((item: any) => (
                                                    <Link
                                                        key={item.name}
                                                        href={item.href}
                                                        className="flex items-center gap-4 text-foreground/30 hover:text-foreground text-[10px] font-black uppercase tracking-[0.25em] transition-all"
                                                        onClick={() => setMobileMenuOpen(false)}
                                                    >
                                                        {item.name}
                                                    </Link>
                                    ...
                                    {session ? (
                                        <div className="space-y-8">
                                            <div className="flex items-center gap-5 px-6 py-5 bg-foreground/5 border border-border rounded-3xl">
                                                <div className="w-10 h-10 rounded-full bg-zinc-800 flex items-center justify-center text-[12px] font-black text-foreground border border-border">
                                                    {session.user.name?.charAt(0) || "U"}
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <p className="text-[12px] font-bold text-foreground/90 truncate leading-none mb-1.5 uppercase tracking-tight">{session.user.name}</p>
                                                    <p className="text-[9px] font-black text-foreground/20 uppercase tracking-[0.1em] truncate">{session.user.email}</p>
                                                </div>
                                            </div>

                                            <div className="grid grid-cols-1 gap-3.5">
                                                {isAdmin && (
                                                    <Link href="/admin" onClick={() => setMobileMenuOpen(false)} className="flex items-center justify-between px-8 py-5 bg-foreground text-background rounded-3xl text-[11px] font-black uppercase tracking-[0.3em] shadow-xl shadow-white/5 transition-all active:scale-95">
                                                        Master Dashboard
                                                        <ShieldCheck size={16} />
                                                    </Link>
                                                )}
                                                {isRegisteredSeller ? (
                                                    <Link href="/seller" onClick={() => setMobileMenuOpen(false)} className="flex items-center justify-between px-8 py-5 bg-foreground/5 border border-border text-foreground rounded-3xl text-[11px] font-black uppercase tracking-[0.3em] transition-all active:scale-95">
                                                        Seller Dashboard
                                                        <BarChart3 size={16} className="text-amber-500" />
                                                    </Link>
                                                ) : (
                                                    <Link href="/become-seller" onClick={() => setMobileMenuOpen(false)} className="flex items-center justify-between px-8 py-5 bg-foreground text-background rounded-3xl text-[11px] font-black uppercase tracking-[0.3em] shadow-xl transition-all active:scale-95">
                                                        Join Global Network
                                            ...
                                            <button
                                                onClick={handleSignOut}
                                                className="w-full flex items-center justify-center gap-4 py-6 border-t border-border text-red-400/60 hover:text-red-400 text-[11px] font-black uppercase tracking-[0.3em] transition-all"
                                            >
                                                <LogOut size={16} />
                                        ...
                                        <div className="grid grid-cols-1 gap-4">
                                            <Link href="/sign-up" onClick={() => setMobileMenuOpen(false)}>
                                                <button className="w-full bg-foreground text-background py-5 rounded-[2rem] text-[11px] font-black uppercase tracking-[0.3em] shadow-2xl transition-all flex items-center justify-center gap-3">
