    ...
    return (
        <section className="relative section-spacing overflow-hidden bg-foreground/[0.01] dark:bg-transparent border-y border-border/50 flex flex-col items-center">
            <div className="container-standard relative z-10 w-full max-w-7xl">
                ...
                <div className="flex flex-col mb-16 pb-12 border-b border-border/50 dark:border-[#27272a]/50">
                     <motion.div
                        initial={{ opacity: 0, y: 10 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        transition={{ duration: 0.6 }}
                        className="inline-flex items-center gap-3 px-4 py-2 rounded-full bg-background dark:bg-foreground/[0.03] border border-border shadow-sm backdrop-blur-md mb-6 w-max"
                    >
                        <div className="w-1.5 h-1.5 bg-secondary rounded-full shadow-[0_0_10px_rgba(var(--secondary),0.8)]" />
                        <span className="text-meta uppercase font-mono tracking-widest text-foreground/60 dark:text-foreground/40">
                    ...
                    <p className="text-foreground/70 dark:text-foreground/40 text-lg lg:text-xl max-w-3xl font-medium leading-relaxed mb-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
                    ...
                    <div className="lg:col-span-7 flex flex-col gap-6">
                        ...
                        <motion.div 
                            ...
                            className="bg-card dark:bg-[#0f0f11] border border-border/50 dark:border-[#27272a] rounded-[2rem] flex flex-col p-6 lg:p-8 h-full min-h-[400px]"
                        >
                            <div className="mb-8">
                                <h3 className="text-xl font-bold text-foreground dark:text-white tracking-tight flex items-center gap-2">
                                    <Cpu className="w-5 h-5 text-cyan-600 dark:text-foreground" />
                                    Profit Unlock Trajectory (Billions)
                                </h3>
                                <p className="text-muted-foreground dark:text-zinc-500 text-xs mt-1">
                                    Agentic AI scaling VS Traditional IT frameworks
                                </p>
                            </div>
                            <div className="flex-grow w-full h-[250px] lg:h-[300px]">
                                <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={revenueData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                        ...
                                        <YAxis stroke="#71717a" fontSize={10} tickLine={false} axisLine={false} tickFormatter={(val) => `$${val}B`} />
                        ...
                        <motion.div 
                            ...
                            className="bg-card dark:bg-[#0f0f11] border border-border/50 dark:border-[#27272a] rounded-[2rem] flex flex-col p-6 lg:p-8 h-full min-h-[350px]"
                        >
                            <div className="mb-8">
                                <h3 className="text-xl font-bold text-foreground dark:text-white tracking-tight flex items-center gap-2">
                                    <ShieldAlert className="w-5 h-5 text-cyan-600 dark:text-foreground" />
                            ...
                            <div className="flex-grow w-full h-[200px] lg:h-[250px]">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={hurdleData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }} barGap={0}>
                                        <CartesianGrid strokeDasharray="3 3" stroke="#27272a" vertical={false} />
                                        ...
                                        <Bar dataKey="cost" fill="#3f3f46" radius={[4, 4, 0, 0]} maxBarSize={40} />
                                        <Bar dataKey="agentic" fill="#39b8f7" radius={[4, 4, 0, 0]} maxBarSize={40} />
                                    </BarChart>
                                </ResponsiveContainer>
                            </div>
                        </motion.div>
                        ...
                        {adoptionDrivers.map((driver, idx) => {
                            ...
                            return (
                                <motion.div 
                                    ...
                                    className="bg-card dark:bg-[#0f0f11] border border-border/50 dark:border-[#27272a] rounded-[2rem] flex flex-col p-6 lg:p-8 group relative overflow-hidden h-full justify-center min-h-[240px]"
                                >   
                                    ...
                                    <div className="relative z-10 flex flex-col h-full">
                                        <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-card dark:bg-[#1a1a1c] border border-border/50 dark:border-[#27272a] group-hover:border-cyan-500/40 dark:group-hover:border-foreground/30 transition-colors mb-6">
                                            <Icon className="h-6 w-6 text-cyan-600 dark:text-foreground" />
                                        </div>
                                        
                                        <h3 className="text-xl font-bold text-foreground dark:text-white mb-3 tracking-tight group-hover:text-cyan-600 dark:group-hover:text-foreground transition-colors">
                                            {driver.title}
                                        </h3>
                                        
                                        <p className="text-muted-foreground dark:text-zinc-400 text-xs md:text-sm leading-relaxed mb-6 flex-grow">
                                            {driver.description}
                                        </p>
                                        
                                        <div className="mt-auto inline-flex items-center gap-3 pt-4 border-t border-border/50 dark:border-[#27272a]/50">
                                            <div className="w-1.5 h-1.5 bg-cyan-600 dark:bg-foreground rounded-full animate-pulse" />
                                            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-cyan-700 dark:text-foreground/80">
                                                {driver.metric}
                                            </span>
                                        </div>
                                    </div>
                ...
                <div className="mt-20 flex justify-center lg:justify-start">
                    <Link href="/services">
                        <button className="bg-cyan-600 hover:bg-cyan-500 dark:bg-foreground dark:hover:bg-foreground/90 text-primary-foreground dark:text-background px-8 py-4 md:px-12 md:py-5 font-black uppercase tracking-[0.2em] text-[10px] md:text-xs rounded-xl transition-all hover:scale-105 active:scale-95 flex items-center gap-3 shadow-[0_0_30px_rgba(57,184,247,0.2)]">
