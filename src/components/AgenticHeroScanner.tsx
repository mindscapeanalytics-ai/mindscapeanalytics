        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.startsWith('|') && line.endsWith('|')) {
                ...
                if (!inTable) {
                    inTable = true;
                    tableRowsCount = 0;
                    currentTableHtml = '<div class="overflow-x-auto my-4 rounded-xl border border-white/10"><table class="w-full text-sm text-left border-collapse">';
                }
                
                const cells = line.split('|').filter((_, index, array) => index !== 0 && index !== array.length - 1);
                
                currentTableHtml += '<tr class="border-b border-white/10 last:border-b-0 hover:bg-white/[0.02] transition-colors">';
                cells.forEach((cell) => {
                    const content = cell.trim();
                    // If it's the first row of the table, treat as header
                    if (tableRowsCount === 0) { 
                         currentTableHtml += `<th class="px-4 py-3 font-bold bg-white/[0.05] border-r border-white/10 last:border-r-0 text-white">${content}</th>`;
                    } else {
